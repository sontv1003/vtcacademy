...
class WSO_Twitter extends WSO_Service
{
	...
	public function publish($postid, $comment = "")
	{
		...
		{
			wso_log_msg(sprintf(__('Error publishing on Twitter:') . ' <a href="%s">post id %d</a> ['.__('Reason:'). ' ' . $response->error . ']',
				get_bloginfo('wpurl')."/wp-admin/post.php?post=".$postid."&action=edit", $postid));
		}
	}
	...
	public function connect()
	{
		...
		else
		{
			...
			$wsoOpts[$this->m_options['options']] = "1|1|1|0";
			$wsoOpts[$this->m_options['format']] = "%title %link %comment %categories %tags";
	...
	foreach($types as $type => $label)
	{
		...
		<tr>
			...
			<td>
				...
				%comment : <?php _e('A comment (you write the comment into the post editor)', 'wordsocial'); ?><br/>
				%categories : <?php _e('Use the post categories as #hashtags', 'wordsocial'); ?><br/>
				%tags : <?php _e('Use the post tags as #hashtags', 'wordsocial'); ?><br/></p>
			</td>
		</tr>
		<tr>
			...
			<td>
				...
				<div id="wso_yourls">
					<?php _e('URL Yourls:', 'wordsocial'); ?> <input type="text" name="wso_yourls_url" value="{$wsoOpts['wsourlp'][0]}" /> (http://your-domain.com/yourls-api.php)<br />
					<?php _e('Signature:', 'wordsocial'); ?> <input type="text" name="wso_yourls_sign" value="{$wsoOpts['wsourlp'][1]}" /><br />
				</div>
				<div id="wso_obitly">
					<?php _e('Login Bit.ly:', 'wordsocial'); ?> <input type="text" name="wso_obitly_login" value="{$wsoOpts['wsourlp'][0]}" /><br />
					<?php _e('API Key:', 'wordsocial'); ?> <input type="text" name="wso_obitly_apikey" value="{$wsoOpts['wsourlp'][1]}" />
				</div>
