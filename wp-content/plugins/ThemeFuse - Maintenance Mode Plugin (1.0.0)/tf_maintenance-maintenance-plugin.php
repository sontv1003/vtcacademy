...
Plugin Name:  TF Maintenance Mode
Plugin URI:   http://www.themefuse.com
Version:      1.0.0
...
**************************************************************************/
 if ( ! defined( 'TF_MAINTENANCE_WP_PLUGIN_URL' ) )
       define( 'TF_MAINTENANCE_WP_PLUGIN_URL', WP_CONTENT_URL. '/plugins/tf_maintenance_mode' );
 if ( ! defined( 'TF_MAINTENANCE_WP_PLUGIN_URL_JS' ) )
       define( 'TF_MAINTENANCE_WP_PLUGIN_URL_JS', WP_CONTENT_URL. '/plugins/tf_maintenance_mode/js' );
...
<div id="message" class="error">
  <p>
    <?php _e( "TF Maintenance Mode  plugin isn't active. Activate it here." ); ?>
  </p>
</div>
<?php }
    ...
    {
        $this->menu_page = add_options_page('TF Maintenance Mode Plugin Options', 'TF Maintenance Plugin', 'manage_options',$this->_options_pagename, array(&$this, 'build_settings_page'));
    ...
    function build_settings_page()
    {
        ...
        if (isset($_REQUEST['saved'])) {
            if ( $_REQUEST['saved'] ) echo '<div id="message" class="updated fade"><p><strong>'.'TF Maintenance'.' settings saved.</strong></p></div>';
	}
    ...
    <table class="form-table-tf">
      <tr valign="top">
        ...
        <td><label for="tf_maintenance_logo">
          <input type="text" class="regular-text" name="tf_maintenance_logo" id="tf_maintenance_logo" value="<?php echo($this->_settings['tf_maintenance_logo']); ?>" />
            ...
            <script>        
        function createUploaderLogo(){            
			var uploader = new qq.FileUploader({
                element: document.getElementById('file-uploader-logo'),
                action: '<?php echo $this->location_folder; ?>/php.php',
				multiple: false,
				onProgress: function(id, fileName, loaded, total){
					jQuery(".qq-upload-list li").hide();
					jQuery(".qq-upload-list li:last-child").show();
				},
				onComplete: function(id, fileName, response){
					jQuery(".qq-upload-list li").hide();
					jQuery("#tf_maintenance_logo").val('<?php echo $this->location_folder; ?>/' + response.url);
				},		  
				debug: false
			});           
        }
      ...
      <tr valign="top">
        ...
        <td><label for="tf_maintenance_bg">
          <input type="text" class="regular-text" name="tf_maintenance_bg" id="tf_maintenance_bg" value="<?php echo($this->_settings['tf_maintenance_bg']); ?>" />
            ...
            <script>        
        function createUploaderBG(){            
			var uploader = new qq.FileUploader({
                element: document.getElementById('file-uploader-bg'),
                action: '<?php echo $this->location_folder; ?>/php.php',
				multiple: false,
				onProgress: function(id, fileName, loaded, total){
					jQuery(".qq-upload-list li").hide();
				...
				onComplete: function(id, fileName, response){
					jQuery(".qq-upload-list li").hide();
					jQuery("#tf_maintenance_bg").val('<?php echo $this->location_folder; ?>/' + response.url);
 				},		  
				debug: false
			});           
        }
      ...
      <tr valign="top">
        ...
        <td><input name="tf_maintenance_complete_text" type="text" id="tf_maintenance_complete_text" value="<?php echo($this->_settings['tf_maintenance_complete_text']); ?>" class="regular-text" />
            <span class="description">
            <?php _e( 'Input the text witch would be visible on the loader bar' ); ?>
          </span> </td>
      </tr>
      <tr valign="top">
        <th scope="row"> <label for="tf_maintenance_date">
...
<?php
    ...
    {	
        ...
        wp_enqueue_script('anticipate-admin-date-addon', $this->location_folder . '/js/jquery-ui-timepicker-addon.js');
 		...
 		wp_enqueue_script('tf_maintenance-admin-upload', $this->location_folder . '/js/fileuploader.js?v=001');
    }
...
<link rel="stylesheet" href="<?php echo $this->location_folder; ?>/css/fileuploader.css?v=2" type="text/css" />
<?php
    }
...
function my_admin_scripts() {
	...
	wp_register_script('my-upload', TF_MAINTENANCE_WP_PLUGIN_URL_JS.'/my-script.js?v=1', array('jquery','media-upload','thickbox'));
