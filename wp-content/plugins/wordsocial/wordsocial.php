...
function wso_publish_post($postid)
{
	...
	if(isset($countMustPublish[1]) && $countMustPublish[1] > 0)
	{
		...
		foreach($wso_services as $servAbbr => $serv)
			$_wso_opts['publishTo'.ucfirst($servAbbr)] = $mustPublish[$servAbbr];
		...
		foreach($wso_services as $servAbbr => $serv)
		{
			$publishargs['servEnable'][$servAbbr] = $_wso_opts['publishTo'.ucfirst($servAbbr)];
			$publishargs['comments'][$servAbbr] = isset($_POST['wso_comment_'.$servAbbr]) ? $_POST['wso_comment_'.$servAbbr] : '';
		}
...
function wso_publish($arr)
{
	...
	foreach($wso_services as $servAbbr => $serv)
		unset($_wso_opts['publishTo'.ucfirst($servAbbr)]);
